...
export default function CreateTeamScreen() {
  ...
  const handleSave = async () => {
    ...
    if (team) {
      router.replace(`/(tabs)/teams/${team.id}`);
    }
  };
    ...
    <View className="flex-1 bg-background dark:bg-surface">
      <View className="pt-12 pb-4 px-4 bg-white dark:bg-gray-900 border-b border-border flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-onSurface dark:text-white">Buat Tim Baru</Text>
        </View>
      </View>
      ...
      <View className="p-4">
        ...
        <View className="mb-4">
          ...
          <TextInput
            ...
            placeholder="Contoh: Kanto Champions"
            value={name}
        ...
        <View className="mb-8">
          ...
          <TextInput
            ...
            placeholder="Tim untuk kompetisi wilayah..."
            value={description}
        ...
        >
          <Text className="text-white font-bold text-lg">{isLoading ? 'Menyimpan...' : 'Simpan Tim'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
