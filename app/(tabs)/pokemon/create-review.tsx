...
export default function CreateReviewScreen() {
  const { pokemonId, pokemonName } = useLocalSearchParams();
  ...
  const handleSave = async () => {
    ...
    const review = await createReview(Number(pokemonId), String(pokemonName), rating, comment);
    if (review) {
      router.back();
    }
  };
    ...
    <View className="flex-1 bg-background dark:bg-surface">
      <View className="pt-12 pb-4 px-4 bg-white dark:bg-gray-900 border-b border-border flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-onSurface dark:text-white">Tulis Review</Text>
        </View>
      </View>
      ...
      <ScrollView className="p-4">
        ...
        <View className="mb-6 items-center">
          <Text className="text-onSurface dark:text-gray-200 mb-2 font-bold text-lg text-center">
            Bagaimana penilaianmu tentang {pokemonName}?
          </Text>
        ...
        >
          <Text className="text-white font-bold text-lg">{isLoading ? 'Mengirim...' : 'Kirim Review'}</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
